...
void Simulator::initializeNetwork()
{
    ...
    m_metricsCollector     = QSharedPointer<MetricsCollector>::create(this);
    ...
    eventsCoordinator->setDataGenerator(m_dataGenerator);
     ...
     * ======================================================
     * ======================================================
     * @attention 1- Set Sender PCs (RingStar PCs)
     * ======================================================
     * ======================================================
     * ======================================================
     * ======================================================
    ...
    connect(m_dataGenerator.data(),
            &DataGenerator::packetsGenerated,
            this,
            &Simulator::handleGeneratedPackets);
}
...
void Simulator::onConvergenceDetected()
{
    ...
    auto executeConvergenceActions = [this]() {
        ...
        qInfo() << "\n\n\n\n\n" << "Next Phase: Packet Sending" << "\n\n\n\n\n";

        connect(eventsCoordinator,
                &EventsCoordinator::thisIsTheEnd,
                this,
                &Simulator::endSimulation);

        QTimer::singleShot(5'000, this, [this]() { initiatePacketSending(); });

        // initiatePacketSending();

        // QTimer::singleShot(5'000, this, [this, &eventsCoordinator]() {
        //     if(m_metricsCollector)
        //     {
        //         m_metricsCollector->printStatistics();
        //         qInfo() << "convergencePhaseDone() is "
        //                 << eventsCoordinator->convergencePhaseDone();
        //     }
        // });
    };
...
void Simulator::initiatePacketSending()
{
    ...
    if (m_dataGenerator) {
        ...
        EventsCoordinator::instance()->startPacketSending();
