...
Q_SIGNALS:
    ...
    void packetsGeneratedForPC(int id, const QList<QSharedPointer<Packet>> &packets);

private:
    QList<QList<QSharedPointer<Packet>>> splitFileToPackets(const QString &filePath,
                                                            size_t packetSize, size_t numParts);
